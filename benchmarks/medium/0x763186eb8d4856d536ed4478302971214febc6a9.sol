...
contract BETR_TOKEN {
    ...
    uint256 public hardCap = 650000000 * (10 ** 18);
    ...
    function escrowFrom(address _from, uint256 _value) external onlyEscrow returns(bool) {
      ...
      balances[_from] = ((balances[_from]) - _value);
      balances[escrow] = ((balances[escrow]) + _value);
    ...
    function escrowReturn(address _to, uint256 _value, uint256 _fee) external onlyEscrow returns(bool) {
        ...
        if(_fee > 0) {
            ...
            totalSupply = (totalSupply - _fee);
            balances[escrow] = ((balances[escrow]) - _fee);
        }
    ...
    function mint(address _user, uint256 _tokensAmount) public onlyTgeIssuer tgeRunning returns(bool) {
        uint256 newSupply = (totalSupply + _tokensAmount);
        require(
            _user != address(0) &&
            _tokensAmount > 0 &&
             newSupply < hardCap
        );
        balances[_user] = ((balances[_user]) + _tokensAmount);
    ...
    function reserveTokensGroup(address[] _users, uint256[] _tokensAmounts) external onlyOwner {
        ...
        for(uint8 i = 0; i < _users.length; i++){
            uint a = (_tokensAmounts[i]) * (10 ** 18);
            newSupply = totalSupply + a;
            require(
                _users[i] != address(0) &&
                _tokensAmounts[i] > 0 &&
                newSupply < hardCap
            );
            balances[_users[i]] = (balances[_users[i]]) + a;
            totalSupply = newSupply;
    ...
    function reserveTokens(address _user, uint256 _tokensAmount) external onlyOwner {
        uint256 newSupply = (totalSupply + ((_tokensAmount * (10 ** 18))));
        require(
            _user != address(0) &&
            _tokensAmount > 0 &&
            newSupply < hardCap
        );
        balances[_user] = ((balances[_user]) + ((_tokensAmount * (10 ** 18))));
        totalSupply = newSupply;
        Transfer(0x0, _user, _tokensAmount);
    }
