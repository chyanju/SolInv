...
class InvariantInterpreter(PostOrderInterpreter):
    ...
    # ======== enum productions ======== #
    # ================================== #
    def eval_EnumInt(self, v):
        return v

    def eval_EnumMap(self, v):
        return v

    # ====================================== #
    # ======== function productions ======== #
    # ====================================== #
    def eval_enum2int(self, node, args):
        arg_enumint = args[0]
        return arg_enumint

    def eval_enum2map(self, node, args):
        arg_enummap = args[0]
        return arg_enummap

    def eval_and(self, node, args):
        arg_bool0, arg_bool1 = args
        return "({} && {})".format(arg_bool0, arg_bool1)

    def eval_leq(self, node, args):
        arg_int0, arg_int1 = args
        return "({} <= {})".format(arg_int0, arg_int1)
    ...
    def eval_flatten(self, node, args):
        arg_mapmap = args[0]
        return "flatten({})".format(arg_mapmap)

    def eval_sum(self, node, args):
        arg_map = args[0]
        return "sum({})".format(arg_map)
